...
\title{Bedside Multimodal Sensing for Early Neurological Risk Assessment after Cardiac Surgery}

\author[1,2]{\fnm{Robin C.} \sur{Geyer}\textsuperscript{*}}

\author[3]{\fnm{Pia} \sur{Lanmueller}\textsuperscript{\textdagger}}

\author[1]{\fnm{Svea} \sur{Wilkending}}

\author[4]{\fnm{Maximilian} \sur{Sch\"{o}ls}}

\author[3]{\fnm{Tobias} \sur{R\"{o}schl}}

\author[3]{\fnm{Julian} \sur{Madrid}}

\author[3]{\fnm{Matthias} \sur{Hommel}}

\author[5]{\fnm{Benjamin} \sur{O'Brien}}

\author[2]{\fnm{Julia} \sur{Vogt}}

\author[3]{\fnm{Volkmar} \sur{Falk}}

\author[2]{\fnm{Joachim M.} \sur{Buhmann}}

\author[1,3]{\fnm{Alexander} \sur{Meyer}}

\affil[1]{\orgdiv{Institute for Artificial Intelligence in Medicine (IKIM)}, \orgname{Charit\'{e} Universit\"atsmedizin Berlin}}

\affil[2]{\orgdiv{Institute for Machine Learning}, \orgname{ETH Zurich}}

\affil[3]{\orgdiv{Department of Cardiothoracic and Vascular Surgery}, \orgname{Deutsches Herzzentrum der Charit\'{e}, Charit\'{e} Universit\"atsmedizin Berlin}}

\affil[4]{\orgdiv{Clinic for Neurology with Experimental Neurology}, \orgname{Charit\'{e} Universit\"atsmedizin Berlin}}

\affil[5]{\orgdiv{Department of Cardiac Anesthesiology and Intensive Care Medicine}, \orgname{Deutsches Herzzentrum der Charit\'{e}, Charit\'{e} Universit\"atsmedizin Berlin}}

\abstract{Neurological complications after cardiac surgery often become clinically apparent only after emergence from anaesthesia, when patients can be examined. Here, we show a bedside multimodal sensing approach for risk stratification during this clinically silent interval. In 154 postoperative cardiac surgery patients, 12 developed neurological complications. We synchronised electronic health records, physiological waveform data and thermo-visual imaging, standardised facial thermal videos into anatomical maps and encoded them with a structured variational autoencoder before temporal prediction with transformer classifiers. Routine electronic health record variables and waveform-derived haemodynamic features did not differ statistically between groups, whereas facial core-to-peripheral and canthal-to-peripheral temperature gradients were higher in complication cases. Models incorporating thermo-visual imaging discriminated neurological complications from the earliest measurements: the trimodal model reached an area under the receiver operating characteristic curve of 0.73 $\pm$ 0.04 at 10 min and 0.79 $\pm$ 0.03 at 90 min after admission. The data associate early facial peripheral cooling, with preserved core temperature, with postoperative neurological risk and motivate external validation of non-contact thermal monitoring.}

\keywords{thermo-visual imaging, multimodal learning, cardiac surgery, neurological complications, postoperative monitoring}

\maketitle

\begingroup
\renewcommand\thefootnote{\fnsymbol{footnote}}
\footnotetext[1]{Technical lead}
\footnotetext[2]{Clinical lead}
\endgroup

Neurological complications after cardiac surgery are associated with increased mortality, morbidity and healthcare use~\citep{arrowsmith2000central}. In the immediate postoperative period, however, patients in the post-anaesthesia care unit (PACU) are typically sedated and mechanically ventilated, which limits neurological examination~\citep{salazar2001stroke, liu2022recent}. Deficits may therefore become apparent only after emergence from anaesthesia, for example as hemiparesis, seizures or delirium, while treatment-sensitive time windows may already be narrowing~\citep{saver2006time, saver2016time, selim2007perioperative}. No established bedside method currently enables neurological risk stratification during this interval~\citep{amundson2020timing}.

One plausible early marker is postoperative sympathetic predominance. Sympathetic predominance can occur after cardiac surgery~\citep{hogue1994alterations, bauernschmitt2004impairment} and has been linked to perioperative neurological complications~\citep{pan2025effect, patel2022potential}. It may reflect processes relevant to neurological injury, including catecholamine release after acute cerebral ischaemia~\citep{jimenez2021cardiovascular, sposato2020post}, impaired cerebral autoregulation~\citep{chen2021autonomic, mankoo2023role, aaslid1989cerebral, lee2019dysfunctional, schramm2012impaired, longhitano2021cerebral} and inflammatory activation associated with delirium or seizures~\citep{mattimore2023delirium, staicu2025postoperative, hansen2025prediction, pongratz2014sympathetic, poredos2025inflammation}. Routine electronic health record variables do not directly capture sympathetic predominance~\citep{seravalle2013assess}.

We therefore evaluated two bedside sensing streams that may carry autonomic or peripheral perfusion information. Physiological waveform data, including electrocardiography, arterial blood pressure and photoplethysmography sampled at 100--300\,Hz, can reflect sympathovagal balance~\citep{colombo2015pulse, valenza2018ecg, adam2023heart, olivieri2024heart}. Thermo-visual imaging can capture non-contact facial temperature patterns; because sympathetic activation can induce peripheral vasoconstriction~\citep{guyenet2006sympathetic, sheng2018crosstalk}, the contrast between cooler distal regions such as the nasal tip and more core-proximal regions such as the inner canthi provides a plausible facial surrogate of peripheral perfusion~\citep{predicting_shock_from_thermal_images, jorge_non-contact_2021, podlasek2025thermography}. We synchronised electronic health records (EHR), physiological waveform data (WAVE) and thermo-visual imaging (TVI) at the bedside and trained temporal machine-learning models to assess early neurological risk.

\section{Results}

\subsection{Synchronised bedside multimodal sensing}

The bedside acquisition system synchronised EHR, WAVE and TVI data streams during the immediate postoperative PACU stay (Fig.~\ref{fig:overview}). EHR data provided low-frequency clinical measurements, laboratory values and treatment variables. WAVE data provided high-frequency electrocardiography, invasive arterial blood pressure and photoplethysmography. TVI provided paired thermal and visual video streams from the patient face.

\begin{figure*}[htbp!]
  \centering
  \includegraphics[width=\textwidth]{figs/Overview.png}
  \caption{Integrated bedside pipeline for early postoperative neurological risk assessment. Thermo-visual imaging (TVI), physiological waveform data (WAVE) and electronic health records (EHR) are synchronised during the post-anaesthesia care unit stay. Each stream is converted into modality-specific features, embedded into a shared latent representation and processed by a temporal transformer classifier for neurological risk prediction.}
  \label{fig:overview}
\end{figure*}

Raw thermal video is high dimensional and affected by patient motion, occlusions and bedside equipment. To reduce these sources of variation, synchronised visual frames were used for three-dimensional face reconstruction, registration to the thermal frame and unwrapping into an anatomically standardised two-dimensional UV map (Fig.~\ref{fig:thermal-pipeline-vae}B). The resulting maps were encoded by a structured variational autoencoder (VAE) trained without diagnostic labels. The encoder separates a latent subspace intended to capture physiological thermal structure from a latent subspace used to model spurious variation; separate decoder branches reconstruct the mean temperature map and estimate pixel-wise variance (Fig.~\ref{fig:thermal-pipeline-vae}C,D).

\subsection{Cohort and neurological outcome}
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{3.0cm}>{\centering\arraybackslash}p{3.9cm}>{\centering\arraybackslash}p{3.9cm}>{\centering\arraybackslash}p{0.85cm}>{\centering\arraybackslash}p{0.85cm}@{}}
    ...
    \midrule
    \multicolumn{5}{l}{\textbf{WAVE biomarkers}} \\
...
\subsection{Thermo-visual imaging improves early risk ranking}

Transformer-based classifiers were trained on all seven modality combinations using 12-fold cross-validation, with one neurological complication case per fold, across seven random seeds. Each modality was projected through learned embedding layers into a shared latent space; the transformer processed samples from admission to the prediction time and returned a binary neurological-risk prediction. VAE splits were matched across modality combinations.

EHR-only, WAVE-only and EHR+WAVE models yielded near-chance area under the receiver operating characteristic curve (AUROC) values across the early observation window (Table~\ref{tab:results_by_time}). Every combination incorporating TVI achieved higher discrimination. At 10\,min after PACU admission, the trimodal model reached an AUROC of 0.73 $\pm$ 0.04, compared with 0.55 $\pm$ 0.03 for EHR, 0.51 $\pm$ 0.03 for WAVE and 0.70 $\pm$ 0.03 for TVI alone. At 90\,min, the trimodal model reached 0.79 $\pm$ 0.03, while TVI alone reached 0.77 $\pm$ 0.02. The corresponding ROC curves and AUROC trajectories showed a consistent separation between models with and without TVI (Fig.~\ref{fig:auroc-roc}).

\begin{table*}[t]
  \caption{
    AUROC from 10 to 90 min after PACU admission for all combinations of EHR, physiological waveforms (WAVE), and thermo-visual imaging (TVI). Values are mean $\pm$ s.d. across seven random seeds. The highest mean AUROC in each row is shown in bold.
  }
  ...
  \begin{tabularx}{\textwidth}{@{}l*{7}{>{\centering\arraybackslash}X}@{}}
    ...
    \textbf{Time} &
    \textbf{EHR} &
    \makecell{\textbf{EHR}\\\textbf{+ WAVE}} &
    \textbf{WAVE} &
    \textbf{TVI} &
    \makecell{\textbf{TVI}\\\textbf{+ EHR}} &
    \makecell{\textbf{TVI}\\\textbf{+ WAVE}} &
...
UV-standardised thermal maps showed lower peripheral facial temperatures in patients with neurological complications, particularly over the nose and cheeks, while bladder-derived core temperature followed a similar warming trajectory in both groups (Fig.~\ref{fig:neuro-temp}). The spatial pattern was most apparent early after PACU admission and diminished over the subsequent hours. The preserved core-temperature pattern argues against systemic hypothermia as the sole explanation and is compatible with altered peripheral facial perfusion.
...
This observational study included adult postoperative cardiac surgery patients monitored in the PACU. The analysis cohort comprised 154 patients with complete data for the neurological complication analysis, including 142 patients without and 12 patients with a postoperative neurological complication. Data were acquired during routine postoperative care using synchronised bedside sensing and electronic record extraction. The study was approved by \textbf{[TODO: insert ethics committee name and approval identifier]}; written informed consent and data-use procedures followed the approved protocol.
...
\begin{figure*}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[anchor=west] at (15.78,7.28) {$\bm{\mu}_{v}$};
    \node[anchor=west] at (15.78,6.23) {$\bm{\sigma}_{v}$};
    \node[anchor=west] at (15.78,4.98) {$\bm{\mu}_{x}$};
    \node[anchor=west] at (15.78,3.93) {$\bm{\sigma}_{x}$};
    \node at (15.82,2.90) {$\mathbf{c}_{\mathrm{TVI}}$};
    ...
    \node at (17.55,6.75) {$\mathbf{z}_{v}$};
    \node at (17.55,4.45) {$\mathbf{z}_{x}$};
  ...
  \caption{Thermo-visual standardisation and structured representation learning.
    ...
    into standardised UV maps, together with a visibility mask. \textbf{C,} A
    ...
    thermal structure. \textbf{D,} Decoder branches estimate pixel-wise
    uncertainty and reconstruct the thermal UV map; adjacent rendered meshes
    illustrate reprojection of UV-space variables onto facial geometry.
...
The VAE was trained without neurological outcome labels. The TVI feature vector at prediction time $t$ was defined as the concatenation of the posterior parameters of the physiological subspace, $\mathbf{c}_{\mathrm{TVI}}(t)=\left[\bm{\mu}_{x}(t)^{\top},\,\bm{\sigma}_{x}(t)^{\top}\right]^{\top}\in\mathbb{R}^{2L_{x}}$.

\subsection{Physiological waveform features}

Heartbeat timestamps were extracted from electrocardiography and photoplethysmography and used to segment the arterial blood pressure waveform into cardiac cycles. For each cycle, five features were extracted: the steepness between diastolic and systolic pressure, curvature near diastolic pressure, curvature near systolic pressure, systolic peak amplitude and a ventricular ejection proxy computed as the area under the systolic waveform. For each prediction time, mean and standard deviation of the cycle-level features were computed within the available temporal window and concatenated into the WAVE feature vector.

\subsection{Electronic health record features}

EHR features included fluid-related variables, laboratory measurements, respiratory parameters, haemodynamic measurements and vasoactive medication variables. Time series were filtered for outliers and artefacts. Missing values were handled by carrying forward the last available measurement where clinically appropriate. Features were standardised within the training data of each cross-validation split before application to validation or test data.

\subsection{Transformer prediction model}

TVI, WAVE and EHR feature vectors were projected through modality-specific embedding layers into 100-dimensional latent spaces. In multimodal configurations, embeddings were concatenated and provided to a shared transformer encoder. The model sampled a fixed-length sequence of 200 timepoints from patient admission to the current prediction time, thereby representing the full observed PACU trajectory at a time-dependent temporal resolution. Absolute positional embeddings encoded elapsed time since PACU admission, and relative positional embeddings encoded temporal distance from the current prediction time. The final encoded sequence was pooled by an attention module and mapped to a binary neurological-risk prediction.

\subsection{Cross-validation and evaluation}

All seven modality combinations were evaluated: EHR only, WAVE only, EHR+WAVE, TVI only, TVI+EHR, TVI+WAVE and TVI+EHR+WAVE. Models were trained with 12-fold cross-validation such that each fold held out one patient with a neurological complication; controls were distributed across folds. Training was repeated across seven random seeds, yielding 84 trained models per modality combination. VAE training splits were matched across modality combinations. Performance was summarized by AUROC at predefined times after PACU admission and by the relative risk rank assigned to each complication case.
...
Further information on research design is available in the Nature Portfolio Reporting Summary linked to this article. \textbf{[TODO: complete and submit the Reporting Summary form.]}
...
The authors thank the clinical, nursing, medical technology and information technology teams at the Deutsches Herzzentrum der Charit\'{e} for supporting bedside data acquisition. \textbf{[TODO: add funding statements and grant numbers according to funder requirements.]}

\section*{Author contributions}

R.C.G., P.L. and A.M. conceived the study. R.C.G. developed the acquisition and analysis pipeline, performed machine-learning analyses and drafted the manuscript. P.L., M.S., T.R., J.M., M.H., B.O., V.F. and A.M. contributed to clinical study design, data acquisition and clinical interpretation. S.W. contributed to data processing and analysis. J.V. and J.M.B. contributed to machine-learning methodology and supervision. R.C.G., P.L. and A.M. interpreted the results. All authors reviewed and approved the manuscript. \textbf{[TODO: confirm author contributions before submission.]}

\section*{Competing interests}

The authors declare no competing interests. \textbf{[TODO: confirm competing interest statement with all authors.]}

\section*{Additional information}

Supplementary information is available for this paper. Correspondence and requests for materials should be addressed to R.C.G. and A.M. \textbf{[TODO: add corresponding author email addresses in the submission system or manuscript metadata.]}
...
For thermal representation learning, the visible face was unwrapped into $64\times64$ UV maps. The visibility mask $\mathbf{m}\in\{0,1\}^{64\times64}$ indicated valid facial measurements, and the temperature map $\mathbf{x}\in\mathbb{R}^{64\times64}$ contained the corresponding thermal values. The structured VAE produced posterior mean and scale vectors for the nuisance and physiological subspaces; each vector ($\bm{\mu}_{v}$, $\bm{\sigma}_{v}$, $\bm{\mu}_{x}$ and $\bm{\sigma}_{x}$) was 50-dimensional. The downstream TVI feature vector used the posterior parameters of the physiological subspace.
...
\begin{table*}[t!]
  \caption{
    Per-patient risk ranks at 90 min after PACU admission for all combinations of EHR, WAVE, and TVI. Values are mean $\pm$ s.d. across seven random seeds. The highest mean rank in each row is shown in bold.
  }
...
\clearpage
\begin{table*}[t]
  \caption{Comparison of the structured VAE with a vanilla VAE for encoding thermo-visual imaging. All three modalities (EHR, WAVE and TVI) are active in both settings. Values are mean $\pm$ s.d. across seven random seeds. The higher mean value in each row is shown in bold.}
